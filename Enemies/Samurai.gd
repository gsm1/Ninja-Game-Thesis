...
func _physics_process(delta):
	...
	if softcollision.is_colliding():
		#εφαρμοζουμε μια ταχυτητα συγκρουσης μεταξυ των σαμουραι ωστε να μην εμφανιζονται σαν ενα σωμα
		velocity += softcollision.get_push_vector() * delta * 850
...
func _on_Hurtbox_area_entered(area):
	...
	#Με την παρακατω εντολη απενεργοποιουμε το hitbox που τυχον εμεινε απο unfinished animation
	$Hitbox.set_collision_mask_bit(2,false)
	hurt  = true
	if stats.health > 0:
		stats.health -= area.damage
		sprite.play("hit")
		hitflag = true #Με αυτην την εντολη κανουμε το flag true, σε περιπτωση που δεν τελειωσε το αντιστοιχο animation επειδη ο εχθρος χτυπηθηκε
...
func _on_Stats_no_health():
	...
	if has_key == true: #Με αυτες τις εντολες κανουμε instance το key οταν πεθανει ο εχθρος
		if KEY:
...
func hit_player():
	...
	if playerrange.can_hit_player() and stats.health > 0 and hurt ==false and hitflag == true: #Ελεγχος οτι ο παιχτης ειναι στο range του attack
		hitflag = false
